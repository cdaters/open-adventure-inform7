...
Resolved status (from `generated/Travel.ni`): historically unresolved rule IDs were `110, 138, 190, 193, 194`; after Milestone 4B, unresolved travel blockers are now effectively down to `138` for runtime dispatch/governance.
...
- In the remaining unresolved rows, the generated row exists with explicit action type and conditions, but behavior is not fully representable in metadata alone.
...
- Keep `special1`, `special2`, and `cond_nodwarves` unresolved rows in generated tables and route through existing handler/predicate surfaces.
...
Remaining unresolved travel effort (post-4B): **6 effort units**
